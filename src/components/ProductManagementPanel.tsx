            ...
            <Card key={product.id} className={`group overflow-hidden transition-shadow hover:shadow-md ${!product.active ? 'opacity-60' : ''}`}>
              ...
              <CardContent className="p-3 space-y-1.5">
                ...
                <div className="flex gap-1 pt-1">
                  ...
                  <Button variant="ghost" size="sm" className="h-7 w-7 p-0" onClick={() => navigate(`/producto/${product.slug || product.id}`)}>
          ...
          {filteredProducts?.map((product: any) => (
            <div key={product.id} className={`flex items-center gap-3 p-3 rounded-lg border border-border bg-background hover:bg-muted/30 transition-colors ${!product.active ? 'opacity-60' : ''}`}>
              ...
              <div className="flex items-center gap-1">
                ...
                <Button variant="ghost" size="sm" className="h-8" onClick={() => navigate(`/producto/${product.slug || product.id}`)}>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div className="space-y-1">
                      ...
                      <CategoryCombobox
                        ...
                        onChange={(val) => setEditingProduct((prev: any) => ({ ...prev, category: val }))}
                      />
                    </div>
                    <div className="space-y-1">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {PLATFORMS.map(p => (
                        <button
                          key={p}
                          type="button"
                          onClick={() => setEditingProduct((prev: any) => ({
                            ...prev,
                            platform: (prev.platform || []).includes(p)
                              ? (prev.platform || []).filter((x: string) => x !== p)
                              : [...(prev.platform || []), p]
                          }))}
                          className={`px-3 py-1.5 rounded-full text-xs font-medium border transition-colors ${
                            (editingProduct.platform || []).includes(p)
                              ? 'bg-primary text-primary-foreground border-primary'
