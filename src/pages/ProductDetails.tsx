...
import { ShoppingCart, Star, ArrowLeft, Share2, Heart, Package, Truck, Shield, CreditCard, MessageCircle, ZoomIn, ZoomOut, RotateCcw, ChevronLeft, ChevronRight, Gift, Loader2 } from 'lucide-react';
...
const ProductDetails = () => {
  ...
  const { user } = useAuth();
  const { createOrder } = useManualOrders();
  ...
  const fetchProduct = async (slugOrId: string) => {
    try {
      ...
      // If it's a UUID, look up by id and redirect to slug
      if (isUUID(slugOrId)) {
        const { data } = await supabase
          .from('products')
          .select('slug')
  ...
  const handleManualOrderSubmit = async () => {
    ...
    const totalValue = getCurrentPrice() * quantity;
    ...
    if (!result.error) {
      ...
      toast({ title: '¡Orden registrada!', description: 'Tu compra fue enviada para validación. Ganarás puntos cuando sea aprobada.' });
    }
  };
            ...
            <div>
              <div className="flex items-start justify-between mb-2">
                <div className="flex flex-col gap-2">
                  ...
                  {product.platform && product.platform.length > 0 && product.platform.map(p => (
                    <Badge key={p} variant="outline" className="w-fit">
                      {p.toUpperCase()}
                    </Badge>
                  ))}
                </div>
                  ...
                  <DialogContent className="sm:max-w-md">
                    ...
                    <div className="space-y-4">
                      <div className="p-3 rounded-lg bg-muted/30 border border-border">
                        <p className="text-sm font-medium">{product.name}{quantity > 1 ? ` x${quantity}` : ''}</p>
                        <p className="text-lg font-bold text-primary">{formatPrice(getCurrentPrice() * quantity)}</p>
                      </div>
                      <div className="space-y-2">
                        <Label>Método de pago</Label>
                        <Select value={manualForm.payment_method} onValueChange={(v) => setManualForm(f => ({ ...f, payment_method: v }))}>
                          <SelectTrigger>
                            <SelectValue />
                          </SelectTrigger>
                          <SelectContent>
                      ...
                      <Button onClick={handleManualOrderSubmit} className="w-full" disabled={manualSubmitting}>
                        {manualSubmitting ? <Loader2 className="h-4 w-4 animate-spin mr-2" /> : <Star className="h-4 w-4 mr-2" />}
                        {user ? 'Enviar orden' : 'Iniciar sesión y enviar'}
                      </Button>
                    </div>
                  </DialogContent>
                </Dialog>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
              {relatedProducts.map((rp) => (
                <Card 
                  ...
                  onClick={() => {
                    navigate(`/producto/${rp.slug || rp.id}`);
                    window.scrollTo(0, 0);
                  }}
                >
